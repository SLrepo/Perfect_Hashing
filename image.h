...
class Color {
public:
  Color(unsigned int red=255, unsigned int green=255, unsigned int blue=255) : r(red),g(green),b(blue) {}
...
class Offset {
public:
  Offset(unsigned int dx_=0, unsigned int dy_=0) : dx(dx_),dy(dy_) {}
  unsigned int dx, dy;
};
...
public:
  ...
  axis operator=(const axis& a) {x=a.x; y=a.y; return *this;}
  int x, y;
};
